...
## color: silahın hasar türü rengi (fiziksel beyazımsı, elementli silahta element rengi).
func setup(facing_cart: Vector2, radius_tiles: float, arc_degrees: float, heavy: bool, color: Color = Color.WHITE) -> void:
	_color = color.lerp(Color.WHITE, 0.35)
	_facing = facing_cart
...
func _draw() -> void:
	...
	if _heavy:
		...
		draw_colored_polygon(poly, Color(_color, 0.55 * alpha))
		...
		draw_colored_polygon(poly2, Color(_color, 0.55 * alpha))
		var outer_edge := Shapes.iso_arc(start_dir, _radius, swept, _radius * 0.98, 16)
		draw_colored_polygon(outer_edge, Color(_color.lerp(Color.WHITE, 0.6), alpha))
